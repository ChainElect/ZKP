...
contract MerkleTreeWithHistory {
    ...
    // @dev Constructor, initialize the tree with the given levels and hasher(address of the hasher contract)
    ...
    // Add this function to the MerkleTreeWithHistory contract
    function updateRoot(bytes32 newRoot) external {    
        uint32 newRootIndex = (currentRootIndex + 1) % ROOT_HISTORY_SIZE;
        currentRootIndex = newRootIndex;
        roots[newRootIndex] = newRoot;

        // Reset or adjust other state variables if necessary
        nextIndex = 0; // Resets the tree, assuming newRoot is for an empty tree
    }
