...
class PoseidonMerkleTree {
    ...
    async initialize() {
        ...
        // Initialize zero values with explicit BigInt conversion
    ...
    getProof(index) {
        ...
        for (let level = 0; level < this.levels; level++) {
            ...
            const sibling = this.leaves[siblingIndex] || this.zeros[level];

            // Explicit BigInt to string conversion
            pathElements.push(this.F.toString(sibling, 10)); // Base 10 conversion
        ...
        return {
            pathElements,
            pathIndices,
            root: this.F.toString(this.getRoot(), 10) // Base 10 conversion
        };
    }
    ...
    getRoot() {
        return this.rootHash || this.zeros[this.levels - 1];
    }
    ...
    getProof(index) {
        ...
        for (let level = 0; level < this.levels; level++) {
            const siblingIndex = currentIndex ^ 1;
            const sibling = this.leaves[siblingIndex] || this.zeros[level];
            pathElements.push(this.F.toString(sibling, 10));
            pathIndices.push(currentIndex & 1);
            currentIndex = Math.floor(currentIndex / 2);
        }
        ...
        return {
            pathElements,
            pathIndices,
            root: this.F.toString(this.getRoot(), 10)
        };
    }
}
